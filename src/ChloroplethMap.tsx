...
const ChoroplethMap = () => {
  ...
  const getColor = (count) =>
    count
      ? chroma.scale('YlOrRd').domain([0, maxCount])(count).hex()
      : '#ccc';

    useEffect(() => {
      // fetch('http://localhost:8000/api/countries/')  // Your Django server serves this path
      //   .then(data => {
      //     console.log(data);  // Check if data is valid
      //     setGeoJsonData(data);  // Use state to store GeoJSON
      //   })
  ...
  const styleFeature = (feature) => ({
    fillColor: "red",//getColor(countryData[feature.properties.ADMIN] || 0),
    weight: 1,
    color: '#333',
    fillOpacity: 0.8,
  });
  ...
  return (
    <MapContainer center={[20, 0]} zoom={2} style={{ height: '600px', width: '100%' }}>
      ...
      {geoJsonData && (
        <GeoJSON data={geoJsonData.features} style={{fillColor: "red"}} />
      )}
    </MapContainer>
  );
};
